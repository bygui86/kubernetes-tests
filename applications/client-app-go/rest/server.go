...
func NewRestServer() (*RestServer, error) {
	...
	// create router
	router := newRouter(cfg.ServerAppConfig)
...
// newRouter -
func newRouter(serverApp *ServerAppConfig) *mux.Router {

	logger.Log.Debugln("[REST] Setup new Router config...")

	router := mux.NewRouter().StrictSlash(false)
	apis := router.PathPrefix("/apis").Subrouter()
	apis.HandleFunc("/users", serverApp.getAll).Methods(http.MethodGet)
	apis.HandleFunc("/users/{email}", serverApp.getByEmail).Methods(http.MethodGet)
	apis.HandleFunc("/users", serverApp.insert).Methods(http.MethodPost)
	apis.HandleFunc("/users", serverApp.update).Methods(http.MethodPut)
	apis.HandleFunc("/users", serverApp.deleteAll).Methods(http.MethodDelete)
	apis.HandleFunc("/users/{email}", serverApp.deleteByEmail).Methods(http.MethodDelete)
	return router
}
